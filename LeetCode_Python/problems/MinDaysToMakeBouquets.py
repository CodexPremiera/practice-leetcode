...
class MinDaysToMakeBouquets:
    ...
    This solution determines the minimum number of days required to make m bouquets,
    each consisting of k adjacent flowers from the bloomDay array.

    Initially, it checks if the total flowers needed exceed the available flowers,
    returning -1 if so.

    It then finds the range of days by identifying the smallest and largest bloom days.
    It uses binary search within this range to efficiently test each day using the
    helper function CanMakeBouquet and ultimately find the minimum day.

    The helper function CanMakeBouquet iterates over the bloom days to count possible
    ...
    """
    def min_days_to_make_bouquets(self, bloom_day: List[int], m: int, k: int) -> int:
        # If the total flowers needed is more than available, return -1
        if m > len(bloom_day) // k:
            return -1
        ...
        def can_make_bouquet(curr_day) -> bool:
            bouquet_count = 0
            flower_count = 0
        ...
        while head <= tail:
            curr_day = (head + tail) // 2

            if can_make_bouquet(curr_day):
                min_days = curr_day
                tail = curr_day - 1  # Search the left half
            else:
                head = curr_day + 1  # Search the right half
